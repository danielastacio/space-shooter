...
public class PlayerController : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        gameSceneController.UpdatedScoreOnKill += GameSceneController_UpdatedScoreOnKill;
    ...
    private void GameSceneController_UpdatedScoreOnKill(int pointValue)
    {
        EnableProjectile();
    }
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        if (collision.gameObject.CompareTag("Power Shield"))
        {
            if (!shieldEnabled)
            {
                EnableShield();
